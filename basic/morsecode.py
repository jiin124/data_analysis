...
def is_validated_morse_code(user_input):
    ...
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당 또는 필요에 따라 자유로운 수정
    result1=None
    userlist=list(user_input)
    usernlist=user_input.split()
    a=["-","."," "]
    b=list(get_morse_code_dict().values())
    count=0
...
def decoding_character(morse_character):
    ...
    mvalue=list(morse_code_dict.values())
    mkey=list(morse_code_dict.keys())

    for i in range(len(mvalue)):
        if(mvalue[i]==morse_character):
            result=mkey[i]
...
def decoding_sentence(morse_sentence):
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당 또는 필요에 따라 자유로운 수정
    mylist=morse_sentence.split(" ")
    mylen=len(mylist)
    result=""

    for i in range(mylen):
        if(mylist[i]!=''):
            result+=decoding_character(mylist[i])
        else:
            result+=' '
...
def encoding_sentence(english_sentence):
    ...
    a=get_cleaned_english_sentence(english_sentence)
    ...
    for i in range(mylen):
        if(mylist[i]!=' '):
            result+=encoding_character(mylist[i])
            result+=' '
        else:
            result+=" "
...
def main():
    ...
    while True:
        ...
        elif(is_help_command(user_input)==False):
            if(user_input=='0'):
                break
            else:
